...
test("月内区域按天和按周独立切换，UTC/本地显示不改变统计", async ({ page }) => {
  await page.goto("/");
  await expect(page.locator(".summary-grid").first().locator(".success-card strong")).toHaveText("30", { timeout: 20_000 });
  ...
  const monthlySection = page.locator(
    'section[aria-labelledby="monthly-distribution-title"]',
  );
  const monthlyChart = monthlySection.locator(".monthly-chart-panel");

  await expect(
    monthlySection.getByRole("heading", { name: "月内高低点日期分布" }).first(),
  ).toBeVisible();
  await expect(monthlyChart.locator(".chart-tick")).toHaveCount(31);
  await expect(monthlyChart.locator(".chart-tick").first()).toContainText("1号");
  await expect(monthlyChart.locator(".chart-tick").last()).toContainText("31号");
  const effectiveMonths = await monthlySection
    .locator(".monthly-summary-grid .success-card strong")
    .textContent();

  await monthlySection.getByRole("button", { name: "按周" }).click();
  await expect(monthlyChart.locator(".chart-tick")).toHaveCount(6);
  await expect(monthlyChart.locator(".chart-tick").first()).toContainText("第1周");
  await expect(monthlyChart.locator(".chart-tick").last()).toContainText("第6周");
  await expect(intradayPanel.getByRole("button", { name: "按小时" })).toHaveClass(/active/);
  await expect(page.locator(".weekly-chart-panel").getByRole("button", { name: /最高点/ })).toHaveClass(/active/);

  await page.getByRole("button", { name: "本地" }).click();
  await expect(monthlySection.locator(".monthly-summary-grid .success-card strong"))
    .toHaveText(effectiveMonths ?? "");
  await expect(monthlyChart.locator(".chart-context")).toContainText("UTC");
  await expect(monthlyChart.locator(".chart-tick")).toHaveCount(6);
});
